//src/compnents/ResourcesSummary
...
export default function ResourcesSummary() {
    ...
    const fetchDashboardSummary = async () => {
        try {
            const response = await fetch('/api/resourcessummary');
    ...
    // Helper function to get button text based on user role
    const getResourceButtonText = () => {
        if (user && user.role === 'field') {
            return 'View Resources';
        }
        return 'Add/Update/Delete Resources';
    };

    // Helper function to handle resource button click
    const handleResourceButtonClick = () => {
        // Navigate to the resources management page 
        navigate('/resources');
    };
    ...
    const renderGroupedSupplies = () => {
        ...
        const suppliesByFacility = suppliesData.reduce((acc, item) => {
            const facilityName = item.facility_name || 'Unassigned Supplies'; 
            if (!acc[facilityName]) {
                acc[facilityName] = [];
            }
            ...
            <div style={{ textAlign: 'right', margin: '20px 0' }}>
                {user && (
                    <button 
                        ...
                        style={styles.resourceManagementButton}
                    >
